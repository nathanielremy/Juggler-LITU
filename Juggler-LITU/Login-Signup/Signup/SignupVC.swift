...
class SignupVC: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    @objc func handlePlusPhoto() {
        let alert = UIAlertController(title: "Professional Photo", message: "You must upload a professional photo of yourself or your application may be denied.", preferredStyle: .alert)
    ...
    @objc func handleTextFieldChanges() {
        ...
        if isFormValid {
            ...
            self.applyButton.backgroundColor = UIColor.mainBlue().withAlphaComponent(1)
        } else {
            self.applyButton.isEnabled = false
            self.applyButton.backgroundColor = UIColor.mainBlue().withAlphaComponent(0.3)
        }
    }
    ...
    @objc fileprivate func handleApply() {
        ...
        guard let profileImage = self.approveProfileImage() else {
            disableAndAnimate(false)
            let alert = UIView.okayAlert(title: "Invalid Profile Image", message: "Please provide a professional profile image.")
